...
class SeamCarver:
    ...
    def object_removal(self):
        ...
        while len(np.where(self.mask[:, :] < 10)[0]) > 0:
            energy_map = self.calc_energy_map() #calculate energy map
            energy_map[np.where(self.mask[:, :] < 10)] *= -self.constant #add mask 
            ...
            print(len(np.where(self.mask[:, :] < 10)[0]))
    ...
    def seams_removal(self, num_pixel):
        if self.protect:
            for dummy in range(num_pixel):
                ...
                energy_map[np.where(self.mask < 10)] *= self.constant
                cumulative_map = self.cumulative_map_forward(energy_map)
                seam_idx = self.find_seam(cumulative_map)
                self.delete_seam(seam_idx)
                self.delete_seam_on_mask(seam_idx)
    ...
    def get_object_dimension(self):
        rows, cols = np.where(self.mask < 10)
