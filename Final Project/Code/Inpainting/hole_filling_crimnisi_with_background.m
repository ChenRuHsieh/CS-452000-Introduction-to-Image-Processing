function [ syn_im ] = hole_filling_crimnisi_with_background( I, target_mask, patch_size, tol, out_file_name, I_bgd )
    figure(5), imshow(I);
    ...
    while any(target_mask(:) == 1)
        ...
        ssd_map = ssd_patch(I_bgd, template); % sdad
        ssd_map = set_forbid_region( ssd_map, target_mask, patch_size );
        patch = choose_sample(ssd_map, tol, I_bgd, patch_size, 0.0001); % dsadda
...
%{
out_file_name = strcat('./results/', out_file_name, '_inpaint.jpg');
